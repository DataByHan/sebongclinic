...
exports.sanitizeNoticeHtml = sanitizeNoticeHtml;
...
const xssFilter = new xss_1.FilterXSS({
    ...
    onTagAttr(tag, name, value) {
        ...
        if (tag === 'a' && name === 'href') {
            ...
            if (!isSafeUrl(trimmed) || /^javascript:/i.test(trimmed))
                return '';
            return `href="${(0, xss_1.escapeAttrValue)(trimmed)}"`;
        }
        if (tag === 'img' && name === 'src') {
            const trimmed = value.trim();
            if (!isSafeUrl(trimmed) || /^javascript:/i.test(trimmed))
                return '';
            return `src="${(0, xss_1.escapeAttrValue)(trimmed)}"`;
        }
        if (tag === 'img' && name === 'data-notice-width') {
            ...
            return `data-notice-width="${(0, xss_1.escapeAttrValue)(`${finalValue}${unit}`)}"`;
        }
